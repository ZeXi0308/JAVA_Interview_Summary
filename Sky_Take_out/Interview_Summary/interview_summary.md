...
TCP (传输控制协议)：**可靠的、面向连接、基于字节流,即：二进制数据，天生没有边界，需要把每条数据包装成消息头｜消息体）的形式**的协议。它会确保数据无差错、不丢失、不重复、且按序到达。就像寄挂号信，必须确认对方签收。


TCP的特点：
- **拥塞控制：**

    拥塞控制是 TCP 协议用来防止网络过载、保证数据传输顺畅的一套机制。
    如果所有人都同时往网络里猛塞数据，网络就会“堵车”（丢包、延迟暴增）。TCP 会根据网络状况动态调整自己的发送速度，防止网络“过载、保证数据传输顺畅。
- **队头阻塞：**

    队头阻塞是一种性能瓶颈现象，发生在队列中的第一个元素（“队头”）受阻，导致其后所有排队的元素都无法被处理，即使这些后继元素本身是可以被处理的。
    **TCP 要保证“数据顺序”**

- **慢启动：**

    慢启动是 TCP 连接建立初期，从很小的发送速率起步，逐步探测最大可用带宽的过程。刚连上网络时，不知道网络能承受多大流量。TCP 先少量发包，没丢就加倍，多了发现丢包就减速，这样既安全又能尽快找到合适速度。
...
核心作用：在分布式系统中生成全局唯一且趋势递增的ID。

agentServer.setId(SnowflakeSingleton.getInstance().nextId()); 这行代码的作用就是：在将新的 AgentServer 记录存入数据库之前，先用雪花算法为它生成一个独一无二的 ID。

**为什么需要雪花算法？**

想象一个场景：你有一个网站，用户量非常大，一台服务器扛不住，所以你部署了10台服务器同时对外提供服务。

- 如果用数据库的自增ID (Auto-increment ID)：当服务器A和服务器B在同一时刻都想创建一个新的 AgentServer 时，它们都去请求数据库生成ID，数据库需要加锁来保证ID不重复，这在高并发下会产生性能瓶颈。

- 如果用随机数 (UUID)：UUID是无序的，用它做数据库主键会导致索引性能很差，因为每次插入都可能导致数据库索引树的大规模调整。

雪花算法就是为了解决这个问题而生的。 它让每一台服务器都能在本地、独立地生成ID，无需请求中央服务器，速度极快，而且保证生成的ID满足以下特点：

- 全局唯一：在全世界、所有服务器、所有时间里，生成的ID都不会重复。

- 趋势递增：ID会随着时间的推移而增大。这对于数据库索引非常友好，可以大大提高插入和查询性能。

**雪花算法ID的构成**

雪花算法生成的ID是一个64位的长整型数字（long），它的内部结构被分成了几个部分，就像一个精心设计的身份证号码：

- 第一部分 (1位)：符号位，永远是0，没有实际意义。

- 第二部分 (41位) - 时间戳：记录了从某个特定时间点开始，到生成ID时所经过的毫秒数。

   这是保证“趋势递增”的关键。因为时间一直在流逝，所以ID的大部分数字会越来越大。

   41位的时间戳大概能用69年。

- 第三部分 (10位) - 机器ID (Worker ID)：这是保证“全局唯一”的关键。

   在部署服务时，会为每一台服务器分配一个独一无二的机器ID（从0到1023）。

   这样，即使两台服务器在同一毫秒生成ID，它们的机器ID部分也不同，所以最终的ID也不同。

- 第四部分 (12位) - 序列号 (Sequence Number)：这是为了解决“同一台服务器在同一毫秒内需要生成多个ID”的情况。

   它是一个计数器，从0开始。如果在同一毫秒内再次请求ID，这个计数器就+1。

   12位意味着一台服务器在1毫秒内最多可以生成 4096个不同的ID，这个性能已经非常惊人了。
   ...
   ```java
      String name = agentServerMapper.getNameById(Long.valueOf(agentServerId), currentAccount.getAccount());
...
数据库查询会尝试寻找一个具有给定 agentServerId 的记录。如果查询一无所获（即 name 为空），就意味着数据库中不存在这样的 Agent Server，自然也就没什么可删除的了。

1. 所有权验证 (Ownership Check) 
请注意，查询语句中包含了 currentAccount.getAccount()。这意味着它不仅仅是在问“是否存在这个ID的Agent Server？”，而是在问“是否存在这个ID的Agent Server，并且它是否属于当前登录的用户？”

   这是一个至关重要的安全措施。它可以防止一个恶意用户（比如用户A）通过猜测ID的方式，去删除另一个用户（用户B）的资源。如果用户A试图删除用户B的资源，查询语句在用户A的账户下将找不到任何匹配项，name 会为空，删除操作也因此会被拒绝。
...
1. **公网IP地址** (Public IP Address) = 你家的家庭住址

    比如“东京都涩谷区惠比寿1丁目2番3号”。

    这个地址是全球唯一的，邮递员（互联网上的其他设备）可以根据这个地址，准确地将信件（数据）送到你家门口。

    任何在“世界”（全球互联网）上的人，只要知道你家的地址，就可以给你寄信。

    这个地址是由官方机构（电信运营商/ISP）分配给你的，你自己不能随便设定。

2. **私有IP地址** (Private IP Address) = 你家里每个房间的“门牌号”

    比如你家路由器给你手机分配的 192.168.1.101，给电脑分配的 192.168.1.102。

    这些地址只在你**自己家里（局域网内）**有意义。你可以在你家墙上贴个“我的卧室”的牌子，你邻居也可以在他家贴一个一模一样的牌子，这完全没问题，因为你们在不同的房子里。

    邮递员是看不到你家“我的卧室”这个牌子的，他只负责把信送到你家的总地址。

    路由器 (Router) 的角色：你家的“收发室/门卫”
    现在问题来了，你用手机（私有IP）上网时，是怎么和外面的世界交流的呢？

3. **路由器**

    这时路由器就扮演了“门卫”的角色，它进行着一个叫做 NAT (网络地址转换) 的关键工作：

    - 出门（发送数据）：你的手机（192.168.1.101）想访问谷歌。它先把请求发给你家的路由器。路由器收到后，会把来源地址从你的私有IP，伪装成它自己的公网IP（你家的家庭住址），然后再发给谷歌。

    - 回家（接收数据）：谷歌把数据回复到你家的公网IP。路由器收到后，会根据它内部的记录，“哦，这个数据是刚才那个手机要的”，然后再把数据准确地转交给你的手机（192.168.1.101）。

    所以，你家里的所有设备（手机、电脑、智能电视）都是共享同一个公网IP地址来上网的。
...
Schema 是数据库的蓝图或骨架，它定义了数据是如何组织、存储和管理的一套结构和规则。当你设计数据库时，你就是在定义它的 Schema。

- **表 (Tables)**： 这是最基本的数据存储单元。Schema 定义了每个表的名称、包含的列（字段），以及每列的数据类型（例如，整数、字符串、日期、布尔值）。

  例子： 一个 Customers 表可能有 CustomerID (整数), FirstName (字符串), LastName (字符串), Email (字符串) 等列。

- **列 (Columns) / 字段 (Fields)**： 定义了表中每个数据的具体属性。Schema 规定了列的名称和它们可以存储的数据类型。

  例子： Email 列的数据类型是 VARCHAR(255)，表示它能存储最长 255 个字符的字符串。

- **数据类型 (Data Types)**： 决定了每列可以存储的数据种类和范围。这对于数据的完整性和存储效率至关重要。

- **关系 (Relationships)**： 如果是关系型数据库，Schema 会定义不同表之间的关联方式，比如通过主键 (Primary Key) 和外键 (Foreign Key) 建立一对一、一对多或多对多的关系。这确保了数据之间的一致性和完整性。

  例子： Orders 表可能有一个 CustomerID 列，它是一个外键，引用了 Customers 表的 CustomerID 主键，表示哪个客户下了哪个订单。

- **约束 (Constraints)：** 规定了数据的有效性和完整性规则。常见的约束包括：

  主键约束 (Primary Key Constraint)： 确保每条记录的唯一性。

  外键约束 (Foreign Key Constraint)： 维护表之间的引用完整性。

  唯一约束 (Unique Constraint)： 确保某列的值在表中是唯一的。

  非空约束 (NOT NULL Constraint)： 确保某列不能为 NULL。

  检查约束 (CHECK Constraint)： 确保某列的值满足特定条件（例如，年龄必须大于 0）。

**视图 (Views)：** 虚拟表，是基于一个或多个基本表的查询结果。Schema 定义了视图的结构，但不存储实际数据。

**索引 (Indexes)：** 提高数据检索速度的机制。Schema 可以包含索引的定义。

存储过程 (Stored Procedures)、函数 (Functions)、触发器 (Triggers) 等： 这些数据库对象也可以是 Schema 的一部分，它们定义了数据处理的业务逻辑。
...
1. **面向云平台设计**
  ...
  应用不是移植到云上，而是为云而生。充分利用云服务（如存储、计算、网络等）的能力。

2. **弹性伸缩**

  可以根据业务需求自动扩展或缩减资源，适应流量变化。

3. **自动化运维**

  应用的部署、升级、故障恢复等流程尽量自动化，减少人工干预。

4. **微服务架构**

  应用被拆分为多个小服务，每个服务可以独立开发、部署和扩展。

5. **容器化部署**

  应用通常以容器（如 Docker）形式打包和部署，保证运行环境一致，易于迁移和扩展。

6. **持续交付与持续部署（CI/CD）**

  代码可以频繁、小步快跑地上线，快速响应需求变化。
...
企业原来在本地机房（物理服务器或虚拟机）上运行应用，把这些应用原封不动搬到云服务器上运行，这叫“上云”或“移植到云上”。

- 这类应用通常还是按传统方式开发和运维，比如大单体应用、依赖固定环境、手动部署等。
- 只是服务器换成了云上的虚拟机，并没有用到云的弹性、自动化等能力。

2. **为云而生**

应用在设计和开发阶段就考虑到要运行在云平台上，充分利用云的分布式、弹性伸缩、自动化运维等优势。

例如
  - 应用拆分为微服务，每个服务可以单独扩展和部署。
  - 应用用容器打包，便于在不同云环境下快速部署。
  - 自动扩容、自动恢复、自动监控，让应用更容易适应流量波动和系统故障。
...
- **面向对象编程（OOP）：**
  是代码层面的设计思想，把系统分成类和对象，每个类/对象负责特定的属性和方法。主要是组织代码和提升可维护性，通常一个进程/应用里包含很多类。


- **微服务架构：**
  是系统层面的架构思想，把一个复杂的系统拆分为多个“服务”，每个服务通常是一个独立的进程/应用，有自己的数据库、部署方式，甚至可以用不同的技术栈实现。服务之间通过网络通信（如HTTP、RPC），分布在不同机器上。

  ![alt text](image-4.png)

- **流程举例**
  1. **开发阶段：**
  你把系统拆成多个微服务，比如订单、用户、商品、支付。
  2. **容器化：**
  每个微服务都用Docker打包成镜像，运行环境标准化。
  3. **部署到K8s：**
  用Kubernetes把这些容器自动部署到云服务器上，自动扩容、健康检查。
  4. **注册到Nacos：**
  每个服务启动时把自己的地址、端口“报到”到Nacos，其他服务需要调用时从Nacos查找，不用写死地址。
  5. **配置统一管理：**
  需要调整参数时，在Nacos改一次，所有服务都能自动感知和更新配置。


### 类和服务的区别
- **类（Class）**：代码级别的概念，是面向对象中的基本单元，一个类通常实现某个功能、数据结构。

- **服务（Service）**：系统架构级别的概念，是一个独立运行的应用，负责一块业务，可以由很多类/模块组成。
...
### 端口的作用是什么？MySQL Workbench 是如何监听的？
- 每台服务器的 IP 地址可以有很多服务，每个服务用不同的端口号区分。
- **MySQL Workbench 作为客户端**，通过 3306 端口发请求到 MySQL 服务。
- 一台计算机可以运行一个 MySQL 服务，也可以配置多个 MySQL 服务进程（端口不一样）。
- 监听端口其实是 MySQL 服务器在监听 3306，等待客户端（比如 Workbench）来连接。
...
- 物理服务器（比如机房中的一台机器）
- 虚拟机（云服务器、虚拟主机等）
- 云数据库服务（阿里云、腾讯云、华为云、AWS、GCP等提供的数据库实例）
- 本地计算机（比如你自己电脑，常用的 127.0.0.1 或 localhost）

总之，IP地址代表一台能被网络访问的设备。
...
“端到端”是一个系统设计和开发理念，它指的是一个系统或模型能够直接从原始输入（一个“端”）接收数据，并直接生成最终输出（另一个“端”），而无需太多中间的人工干预、复杂的特征工程或多个独立的子系统协作。

想象一下一条生产线：

- 非端到端（传统方式）： 就像一条生产线上有很多独立的工位。每个工位只负责一个特定的步骤（比如，先是“清洗部件”，然后是“组装A”，再是“组装B”，最后是“喷漆”）。每个工位之间可能需要人工传递，或者需要专门的接口来衔接，而且每个工位都需要独立优化。

- 端到端： 就像一个高度自动化的智能工厂。你把原材料从一端放进去，机器自动完成所有工序，从另一端直接出来一个成品。中间的所有复杂处理和决策都由这个单一的、集成的系统来完成。
...
- B+树是一种多路平衡查找树，所有的值都存储在叶子节点，内部节点只存储索引。
- 节点设计为能容纳多条记录（如InnoDB默认一个页16KB，能存几百条索引），极大减少树的高度。
- 每个节点在磁盘上是连续存储的（页结构），一次IO可读出整个节点。

**跳表**

- 跳表是一种分层有序链表结构，查找平均复杂度O(log n)。
- 每一层本质是链表，节点通常分布在各个磁盘页上，每跳一次可能换一个磁盘页。跳表的每个节点通常是单独存储的，彼此之间没有局部性。
- 跳表每访问一个节点，理论上都可能是一次磁盘IO。
...
- 读写锁（Read-Write Lock） ：允许多个线程同时读取共享资源，但只有一个线程可以对共享资源进行写操作。信号量(Semaphore) ：它允许同一时刻多个线程访问同一资源，但是需要控制同一时刻访问此资源的最大线程数量。
```java
...
### synchronized(obj) { ... } 锁对象到底是什么意思？

**这个对象的主要作用就是充当一把“锁”。** 对象只是用来充当锁的标识，线程同步靠的是对这把锁（也就是对象的monitor）的互斥访问。

**1. 锁住的不是“对象的资源”，而是“对象的锁”**

每个Java对象天生都自带一把“锁”（也叫“监视器锁”monitor lock）。
synchronized(obj)的意思是：
线程进入这段代码时，必须先获得obj这把锁；
如果别的线程已经获得了obj的锁，当前线程就会等待，直到锁被释放。

**2. 锁对象的作用范围**

不是锁住对象的内容或属性，而是**以obj为标识的代码块互斥**。
只有synchronized(obj)块之间会互斥，只和synchronized(obj)相关。

---

### 什么是PCB

PCB（Process Control Block） 即进程控制块，是操作系统中用来**管理和跟踪进程的数据结构**，每个进程都对应着一个独立的 PCB。你可以将 PCB 视为进程的大脑。当操作系统创建一个新进程时，会为该进程分配一个**唯一的进程 ID**，并且为该进程创建一个对应的进程控制块。当进程执行时，PCB 中的信息会不断变化，操作系统会根据这些信息来管理和调度进程。

PCB 主要包含下面几部分的内容：
- 进程的描述信息，包括进程的名称、标识符(PID)等等；多个进程的名称可以重复但是PID只能有一个。
...
- 消息队列(Message Queuing) ：**消息队列是消息的链表**,具有特定的格式,存放在内存中并由消息队列标识符标识。管道和消息队列的通信数据都是先进先出的原则。与管道（无名管道：只存在于内存中的文件；命名管道：存在于实际的磁盘介质或者文件系统）不同的是**消息队列存放在内核中**，只有在内核重启(即，操作系统重启)或者显式地删除一个消息队列时，该消息队列才会被真正的删除。消息队列可以实现消息的随机查询,消息不一定要以先进先出的次序读取,也可以按消息的类型读取.比 FIFO 更有优势。消息队列克服了信号承载信息量少，管道只能承载无格式字 节流以及缓冲区大小受限等缺点。
...
在 Unix/Linux 系统中，子进程通常是通过 fork()系统调用创建的，该调用会创建一个新的进程，该进程是原有进程的一个副本。子进程和父进程的运行是相互独立的，它们各自拥有自己的 PCB，即使父进程结束了，子进程仍然可以继续运行。当一个进程调用 exit()系统调用结束自己的生命时，内核会释放该进程的所有资源，包括打开的文件、占用的内存等，但是该进程对应的 PCB 依然存在于系统中。

这些信息只有在父进程调用 wait()或 waitpid()系统调用时才会被释放，以便让父进程得到子进程的状态信息。这样的设计可以让父进程在子进程结束时得到子进程的状态信息，并且可以防止出现“僵尸进程”（即子进程结束后 PCB 仍然存在但父进程无法得到状态信息的情况）。
...
- （资源）互斥：**资源必须处于非共享模式**，即一次只有一个进程可以使用。如果另一进程申请该资源，那么必须等待直到该资源被释放为止。
- 占有（资源）并等待：一个进程至少应该占有一个资源，并等待另一资源，而该资源被其他进程所占有。- 非抢占：资源不能被抢占。只能在持有资源的进程完成任务后，该资源才会被释放。
- 循环等待：有一组等待进程 {P0, P1,..., Pn}， P0 等待的资源被 P1 占有，P1 等待的资源被 P2 占有，……，Pn-1 等待的资源被 Pn 占有，Pn 等待的资源被 P0 占有。

---

### Linux系统里的inode和block

在 Linux 文件系统中：

- inode (索引节点)：是一个记录文件“元数据”（metadata）的数据结构。它就像一个文件的“身份证”或者“属性表”，记录了文件的大小、权限、所有者、创建修改时间，以及最重要的——指向存储文件实际内容的数据块 (block) 的地址。inode 本身不存储任何文件内容。

- block (数据块)：是文件系统中用于实际存储文件内容的最小单位。操作系统对硬盘的读写都是以 block 为单位进行的。一个文件的数据会被保存在一个或多个 block 中。

核心关系：操作系统通过文件的 inode 找到其对应的 block，然后才能读取或写入文件的实际数据。一个 inode 维护一个文件的属性，而多个 block 存储这个文件的内容。

- 目录 (Directory): 一种特殊的文件，它的 block 里存放的不是普通数据，而是一张列表，记录了该目录下所有文件名和其对应 inode 号码的映射关系。


**SUPPLEMENT：**
inode 的总数在格式化文件系统时就固定了。这就是为什么你可能会遇到 “disk space full” 的错误，但用 df -h 查看发现磁盘空间还剩很多。这时你应该用 df -i 命令来查看 inode 的使用情况，很可能 inode 的使用率已经达到了 100%。这种情况在需要存储大量小文件的应用场景中（比如邮件服务器、爬虫缓存）时有发生。

---

### 硬链接和软链接

- 硬链接 (Hard Link): 本质上是给一个文件起了个别名。它和原文件共享同一个 inode 号，因此在文件系统层面，它们被视为完全相同的文件。硬链接不能跨文件系统，也不能链接到目录。

- 软链接 (Symbolic Link): 本质上是一个全新的、独立的文件，它有自己专属的 inode 号。这个文件的内容很特殊，它存储的是另一个文件的路径。它就像 Windows 系统里的快捷方式。软链接可以跨文件系统，也可以链接到目录。

#### 为什么需要链接？

在编程和系统管理中，我们经常有这样的需求：

- 共享：让同一个文件出现在多个目录中，而不用复制多份，浪费磁盘空间。

- 版本管理：比如你的服务器上运行着一个程序 app-v1.0.jar。当你要升级到 app-v2.0.jar 时，你可以只修改一个软链接 current-app.jar，让它从指向 v1.0 改为指向 v2.0，而不需要修改启动脚本。回滚也同样方便。

- 简化路径：当一个常用文件的路径非常深时（如 /a/b/c/d/e/config.xml），可以在一个方便的位置创建一个链接，简化访问。

为了满足这些需求，就诞生了硬链接和软链接这两种机制。
...
让我们用一个具体的例子来理解“跨文件系统”：

#### 场景

- 你的电脑有一个主硬盘，被格式化为 **ext4** 文件系统，挂载到根目录 `/`。
- 你插入了一个 U盘，格式化为 **FAT32** 文件系统，挂载到 `/mnt/usb`。
...
#### 操作一：不跨文件系统（在同一仓库内部整理）

例如执行：

```bash
mv /home/user/document.txt /tmp/
```

- /home/user 和 /tmp 都在主硬盘（ext4）上，属于同一个文件系统。
- 文件系统只需要：
  - 找到 document.txt 的 inode。
  - 修改 inode 的“父目录”信息（从 /home/user 改为 /tmp）。

- 文件的数据块没有实际移动。
类比：图书管理员把一本书的索引卡从“小说区”移到“新书区”，书本身还在原来的书架上。


#### 操作二：跨文件系统（从一个仓库搬到另一个仓库）

例如执行：

```bash

mv /home/user/document.txt /mnt/usb/
```

这时，文件要从 ext4 移动到 FAT32。

ext4 的“管理员”无法直接操作 FAT32 的“管理员”。

操作系统需要进行 复制 + 删除 的过程：

- 复制（Copy）：
在目标文件系统（U盘 FAT32）上创建全新文件，分配 inode 和 block，把源文件的数据逐字节读取出来，再写入 U盘。
- 删除（Remove）：
复制成功后，在源文件系统（主硬盘 ext4）上删除原始文件。
这也是为什么：

  - 在同一硬盘分区内移动大文件几乎瞬间完成（只改索引）。
  - 移动到 U盘则耗时很长（数据需真实读写）。

**补充说明**：为什么硬链接不能跨文件系统？
- 硬链接：是指多个目录项指向同一个 inode。
- inode 号只在所属文件系统内部有意义，不能跨文件系统引用。
- 类比：你无法在一个仓库的清单上，引用另一个仓库的货物。